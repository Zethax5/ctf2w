...
new Float:ProjectilesBounce_Damage[MAXPLAYERS + 1][MAXSLOTS + 1];
new Float:ProjectileBounces_Damage[2049];
...
new Float:Earthquake_MinFalloff[MAXPLAYERS + 1][MAXSLOTS + 1];
new Float:Earthquake_Pushscale[MAXPLAYERS + 1][MAXSLOTS + 1];
...
new ShareDamageActive[MAXPLAYERS + 1][MAXSLOTS + 1];
...
new Float:DrawMiniCrits_Accuracy[MAXPLAYERS + 1][MAXSLOTS + 1];
new Float:DrawMiniCrits_Dur[MAXPLAYERS + 1][MAXSLOTS + 1];
new DrawMiniCrits_x10[MAXPLAYERS + 1][MAXSLOTS + 1];
...
public Action:CW3_OnAddAttribute(slot, client, const String:attrib[], const String:plugin[], const String:value[], bool:whileActive)
{
	...
	} else if(StrEqual(attrib, "projectiles bounce", false))
	{
		...
		ProjectilesBounce_Count[client][slot] = StringToFloat(values[0]);
		ProjectilesBounce_Damage[client][slot] = StringToFloat(values[1]);
	...
	} else if(StrEqual(attrib, "earthquake on rocket jump land", false))
	{
		...
		Earthquake_Range[client][slot] = StringToFloat(values[0]);
		Earthquake_Damage[client][slot] = StringToFloat(values[1]);
		Earthquake_MinFalloff[client][slot] = StringToFloat(values[2]);
		Earthquake_Pushscale[client][slot] = StringToFloat(values[3]);
	...
	} else if(StrEqual(attrib, "share damage"))
	{
		...
		ShareDamageActive[client][slot] = StringToInt(values[2]);
	...
	} else if(StrEqual(attrib, "on switch give and take mini crits"))
	{
		...
		DrawMiniCrits_Value[client][slot] = StringToFloat(values[0]);
		DrawMiniCrits_Accuracy[client][slot] = StringToFloat(values[1]);
		DrawMiniCrits_x10[client][slot] = StringToInt(values[2]);
...
public CW3_OnWeaponRemoved(slot, client)
{
	if(Client_IsValid(client))
	{
		...
		ProjectilesBounce_Damage[client][slot] = 0.0;
		...
		Earthquake_MinFalloff[client][slot] = 0.0;
		Earthquake_Pushscale[client][slot] = 0.0;
		...
		ShareDamageActive[client][slot] = 0;
		...
		DrawMiniCrits_Accuracy[client][slot] = 0.0;
		DrawMiniCrits_x10[client][slot] = 0;
...
public Action:Attributes_ProjCreatedPost(Handle:hTimer, any:entity)
{
	...
	if(Client_IsValid(owner))
	{
		...
		ProjectileBounces_Damage[entity] = ProjectilesBounce_Damage[owner][slot];
...
stock Action:Attribute_1040_PStartTouch(entity, other, owner, slot)
{
	...
	{
		g_iProjectileBounces[other] = g_iProjectileBounces[entity];
		ProjectileBounces_Damage[other] = ProjectileBounces_Damage[entity];
	}
...
public Action:Attributes_WeaponSwitch(client, weapon)
{
	...
	if(slot == -1) return Plugin_Continue;
	if(HasAttribute(client, _, DrawMiniCrits) && !DrawMiniCrits[client][slot])
	{
		if(GetAttributeValueI(client, _, DrawMiniCrits, DrawMiniCrits_x10) == 0)
		{
			TF2_RemoveCondition(client, TFCond:19);
			TF2_RemoveCondition(client, TFCond_MarkedForDeath);
		}
		if(GetAttributeValueI(client, _, DrawMiniCrits, DrawMiniCrits_x10) == 1)
		{
			TF2_RemoveCondition(client, TFCond:56);
			TF2_RemoveCondition(client, TFCond_MarkedForDeath);
		}
	}
	...
	{
		if(DrawMiniCrits_x10[client][slot] == 0)
		{
			TF2_AddCondition(client, TFCond:19, DrawMiniCrits_Value[client][slot]);
			TF2_AddCondition(client, TFCond_MarkedForDeath, DrawMiniCrits_Value[client][slot]);
		}
		if(DrawMiniCrits_x10[client][slot] == 1)
		{
			TF2_AddCondition(client, TFCond:56, DrawMiniCrits_Value[client][slot]);
			TF2_AddCondition(client, TFCond_MarkedForDeath, DrawMiniCrits_Value[client][slot]);
		}
		TF2Attrib_SetByName(GetPlayerWeaponSlot(client, slot), "weapon spread bonus", DrawMiniCrits_Accuracy[client][slot]);
		DrawMiniCrits_Dur[client][slot] = GetEngineTime();
	}
...
stock Attributes_PreThink(client)
{
	...
	if(slot > -1 && DrawMiniCrits[client][slot] && GetEngineTime() >= DrawMiniCrits_Dur[client][slot] + DrawMiniCrits_Value[client][slot])
	{
		TF2Attrib_RemoveByName(GetPlayerWeaponSlot(client, slot), "weapon spread bonus");
	}
}
	...
	for(new teammate = 0; teammate <= MaxClients; teammate++)
	{
		if(Client_IsValid(teammate) && IsClientInGame(teammate) && IsPlayerAlive(teammate) && team == GetClientTeam(teammate))
		{
			...
			if(!TR_DidHit(INVALID_HANDLE))
			{
				...
				if(slotTank != -1 && m_bHasAttribute[teammate][slotTank] && ShareDamage[teammate][slotTank] && ShareDamageActive[teammate][slotTank] == 1 && Entity_GetDistanceOrigin(teammate, vClientPos) <= ShareDamage_Range[teammate][slotTank] && health > 1)
				{
					tank[totalTanks] = teammate;
					totalTanks++;
				}
				else if(GetAttributeValueI(teammate, _, ShareDamage, ShareDamageActive) == 0 && Entity_GetDistanceOrigin(teammate, vClientPos) <= GetAttributeValueF(teammate, _, ShareDamage, ShareDamage_Range) && health > 1)
				{
					tank[totalTanks] = teammate;
					totalTanks++;
				}
...
stock Action:Attribute_1066_OnTakeDamage(victim, &attacker, slot, &Float:damage, &damagetype, Float:damageForce[3], Float:damagePosition[3], bool:bBuilding)
{
	...
	if((dmgType & DMG_FALL) == DMG_FALL || (dmgType & DMG_CRUSH) == DMG_CRUSH)
	{
...
public Action:Attributes_OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damageType, &weapon, Float:damageForce[3], Float:damagePosition[3], damageCustom)
{
	...
	if(ProjectileBounces_Damage[victim] > 0.0)
	{
		damage *= ProjectileBounces_Damage[victim] * g_iProjectileBounces[victim] + 1.0;
		ProjectileBounces_Damage[victim] = 0.0;
		g_iProjectileBounces[victim] = 0;
		aReturn = Plugin_Changed;
	}
